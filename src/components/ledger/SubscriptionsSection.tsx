...
import { Tv, Plus, Trash2, X, CreditCard, Banknote, Edit2, Check, Gift } from 'lucide-react';
...
export function SubscriptionsSection({ subscriptions, monthData, cards, onAdd, onEdit, onSoftDelete, onTogglePaid, onUpdateValue }: Props) {
  ...
  const total = subscriptions.reduce((acc, sub) => acc + (monthData.subscriptionValueOverrides[sub.id] ?? sub.value), 0);
  ...
  return (
    <section className="ledger-card p-6 flex flex-col h-full">
          ...
          {subscriptions.map(sub => {
            ...
            return (
              <motion.div key={sub.id} className="flex items-center gap-2 group min-w-0">
                {isPix ? <input type="checkbox" checked={isPaid} onChange={() => onTogglePaid(sub.id)} className="ledger-checkbox shrink-0" /> : <div className="w-4 h-4 shrink-0 flex items-center justify-center">{isFree ? <Gift size={12} className="text-muted-foreground/50" /> : <CreditCard size={12} className="text-muted-foreground/50" />}</div>}
                <div className="flex-1 min-w-0 flex flex-col"><span className={`text-sm truncate ${(isPaid && isPix) ? 'line-through text-muted-foreground' : 'text-foreground'}`}>{sub.name}</span><span className="flex items-center gap-1 text-[10px] text-muted-foreground">{isPix && `Pagamento: dia ${sub.dueDay} • `}{cardName}</span></div>
                <div className="relative shrink-0 w-24"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground text-xs">R$</span><CurrencyInput placeholder="0,00" className="ledger-input w-full font-mono pl-7" value={currentValue || 0} onChange={val => onUpdateValue(sub.id, val)} /></div>
                <div className="flex opacity-0 group-hover:opacity-100 transition-opacity shrink-0"><button type="button" onClick={() => startEdit(sub)} className="p-1.5 text-muted-foreground hover:text-primary"><Edit2 size={14} /></button><button type="button" onClick={() => onSoftDelete(sub.id)} className="p-1.5 text-muted-foreground hover:text-destructive"><Trash2 size={14} /></button></div>
              </motion.div>
            );
          })}
        </AnimatePresence>
      ...
      {subscriptions.length > 0 && <div className="mt-4 pt-4 border-t border-border flex justify-between items-center text-sm"><span className="font-medium text-muted-foreground">Total:</span><span className="font-bold text-foreground">{formatCurrency(total)}</span></div>}
    </section>
  );
}