...
interface Props {
  recurring: RecurringExpense[]; monthData: MonthData;
...
export function RecurringExpensesSection({ recurring, monthData, onAdd, onEdit, onSoftDelete, onTogglePaid }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); if (!form.name || !form.value) return; onAdd(form.name, form.value, Number(form.dueDay) || 1); setForm({ name: '', value: 0, dueDay: '' }); setShowForm(false); };
  const startEdit = (re: RecurringExpense) => { setEditingId(re.id); setEditForm({ name: re.name, value: re.value, dueDay: String(re.dueDay) }); };
      ...
      <div className="space-y-2 flex-1">
        <AnimatePresence initial={false}>
          {recurring.map(re => {
            ...
            if (isEditing) {
              return (
                <motion.div key={re.id} className="p-3 bg-muted/30 border border-border rounded-lg space-y-2"><input type="text" className="ledger-input w-full text-xs" value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} /><div className="flex gap-2"><CurrencyInput className="ledger-input w-full font-mono text-xs" value={editForm.value} onChange={val => setEditForm(f => ({ ...f, value: val }))} /><input type="number" className="ledger-input w-16 text-xs text-center" value={editForm.dueDay} onChange={e => setEditForm(f => ({ ...f, dueDay: e.target.value }))} /></div><div className="flex gap-2 mt-2"><button onClick={() => saveEdit(re.id)} className="flex-1 ledger-btn-primary py-1.5 text-xs flex justify-center items-center gap-1"><Check size={14} /> Salvar</button><button onClick={() => setEditingId(null)} className="flex-1 ledger-btn-outline py-1.5 text-xs flex justify-center items-center gap-1"><X size={14} /> Cancelar</button></div></motion.div>
              );
            }
              ...
              <motion.div key={re.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg border border-border group mb-2">
                <div className="flex items-center gap-3 overflow-hidden">
                  <div className="w-4 flex items-center justify-center shrink-0">
                    <input type="checkbox" checked={isPaid} onChange={() => onTogglePaid(re.id)} className="ledger-checkbox" />
                  </div>
