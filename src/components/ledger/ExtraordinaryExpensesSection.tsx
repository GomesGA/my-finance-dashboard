...
export function ExtraordinaryExpensesSection({ expenses, onAdd, onUpdate, onRemove }: Props) {
  return (
    <section className="ledger-card p-6">
      ...
      <div className="space-y-2">
          ...
          {expenses.map(exp => (
            ...
            >
              ...
              <input
                ...
                className={`ledger-input flex-1 min-w-0 ${exp.paid ? 'line-through text-muted-foreground' : ''}`}
                value={exp.name}
        ...
        {expenses.length === 0 && (
          <p className="text-xs text-muted-foreground italic py-2">Nenhuma despesa extraordinária.</p>
        )}
      </div>
    </section>
  );
