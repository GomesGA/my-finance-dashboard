        ...
        <AnimatePresence initial={false}>
          {cards.map(card => (
            <motion.div key={card.id} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-2 group min-w-0">
              ...
              <input type="number" placeholder="Dia" className="ledger-input w-14 shrink-0 text-center text-xs" value={card.dueDay || ''} onChange={e => onUpdate(card.id, { dueDay: Number(e.target.value) })} min={1} max={31} title="Dia de vencimento" />
